  ...
  return (
    <nav className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-50">
      ...
      <div className="flex items-center gap-4 text-sm">
        <Link href="/providers" className="text-gray-600 hover:text-green-600 hidden sm:block">
          Buscar prestadores
        </Link>
        ...
        {user ? (
          <>
            {profile?.role === 'provider' && (
              <Link href="/requests" className="text-gray-600 hover:text-green-600 hidden sm:block">
                Pedidos
              </Link>
            )}
            <Link
